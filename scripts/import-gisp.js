...
//
//   node scripts/import-gisp.js ВЫГРУЗКА.xlsx [ключи]
//
// Имя файла подставляется как есть. Угловых скобок в команде быть не должно:
// оболочка примет их за перенаправление ввода и скажет «No such file».
//
...
// Куда пишется база в текущем запуске. Нужно обработчику ошибок: недописанную
// базу надо убрать, а он про args ничего не знает.
//
// Это ВРЕМЕННОЕ имя, а не окончательное: собранная база встаёт на место старой
// последним действием, переименованием. Раньше старая удалялась ПЕРЕД сборкой,
// и неудачный импорт — кончилось место, оборвали с клавиатуры, съехали
// колонки — оставлял сервер вообще без реестра. Плата за это: пока идёт
// сборка, на диске лежат обе базы.
let outPath = null;

/** Имя, под которым база собирается. Рядом с окончательным — переименование
 *  внутри одной папки мгновенно и не может оборваться на полпути. */
const buildName = (out) => `${out}.new`;

async function main() {
  const args = parseArgs(process.argv.slice(2));
  outPath = buildName(args.out);

  if (!args.file) {
    console.error(
      "Укажите файл выгрузки:\n" +
        "  node scripts/import-gisp.js ВЫГРУЗКА.xlsx [--dry-run] [--out путь]\n" +
  ...
  const finish = (mapping, rowsSeen, db, stats) => {
    if (args.stats) {
      ...
      const CLASS_NAMES = {
        "05": "уголь", "06": "нефть и газ (добыча)", "07": "руды металлические",
        "08": "прочие ископаемые", "09": "услуги в добыче",
        "10": "пищевые продукты", "11": "напитки", "13": "текстиль",
        "14": "одежда", "15": "кожа", "16": "дерево", "17": "бумага",
        "18": "печать", "19": "нефтепродукты",
        ...
        "31": "мебель", "32": "прочая продукция", "33": "ремонт и монтаж",
    ...
    // Собранная база встаёт на место старой. Переименование внутри одной папки
    // либо произошло целиком, либо не произошло вовсе, — промежуточного
    // состояния, в котором сервер увидел бы полубазу, не бывает.
    fs.renameSync(outPath, args.out);
  ...
  const cleanup = (signal) => {
    ...
    try {
      ...
      console.error(`\n\nИмпорт прерван. Удалите ${outPath} вручную: ${e.message}`);
    }
    process.exit(130);
  };
    ...
    if (writing) {
      ...
      console.log(
        `\nПишем базу в ${args.out}. Это займёт минуту-две.` +
          (fs.existsSync(args.out)
            ? `\nСтарая база остаётся на месте до конца сборки: новая собирается` +
              `\nрядом, под именем ${path.basename(outPath)}, и встаёт на место последним шагом.`
            : ""),
      );
    }
  };
...
main().catch((e) => {
  ...
  if (e.code === "ENOSPC" || e.code === "SQLITE_FULL") {
    ...
    try {
      ...
      // Не вышло убрать — скажем об этом ниже вместе с остальным.
    }
  ...
  console.error(
    `Импорт не удался: ${e.message}\n` +
      (outPath && fs.existsSync(outPath)
        ? `Прежняя база не тронута. Недописанную уберите: rm ${outPath}`
        : "Прежняя база не тронута."),
  );
  process.exit(1);
});
