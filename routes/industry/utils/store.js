...
function lookupProduct(rawName, opts) {
  ...
  // Подпись-заготовка — не вещество, и спрашивать про неё реестр нельзя.
  //
  // Вопрос не безобиден. «Новый продукт» ступень «значимые слова» сводит к
  // одному слову «новый» — «продукт» из ядра выбрасывается как общее, — а в
  // реестре полно торговых имён вроде «Жироудалитель „Новый Элемент“». Слово
  // совпадает вторым, покрывает треть записи, и отбор такое подтверждает.
  // Узел-заготовка получал чужого производителя, чужой ОКПД2 и ярлык
  // «Найден в реестре ПП №719».
  //
  // Сверяем название ЦЕЛИКОМ, а не по вхождению слова: «Новый продукт» —
  // заготовка, а «Полиэтилен новый» — возможное название, и вырезать из него
  // слово нельзя. По той же причине «новый» не отправлен в GENERIC_WORDS:
  // там он противопоставлен «вторичному», и первичный полиэтилен слился бы
  // со вторичным сырьём.
