...
const SUPPLY_FORM = new Set(
  ["стандарт", "титр", "фиксанал", "набор", "комплект", "тест"].map(stemWord),
);
...
function keepBestOverlap(rows, rawName) {
  ...
  const scored = rows.map((row) => {
    ...
    return {
      ...
      // Запись начинается с формы поставки: продукт — набор или мерный
      // реактив, а вещество в нём лишь содержимое.
      supplyForm:
        rowList.length > 0 &&
        SUPPLY_FORM.has(rowList[0]) &&
        !queryStems.has(rowList[0]),
    };
  });
