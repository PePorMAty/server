...
Если по продукту нет явного forward-описания — он НЕдостаточен. Сомневаешься — считай НЕдостаточным (лучше добрать источники, чем молча замкнуть цепочку в цикл).

Верни СТРОГО JSON по схеме: { "insufficient": ["продукт", ...] }. Если для ВСЕХ продуктов есть forward-описание — { "insufficient": [] }. Имена продуктов возвращай ТОЧНО как даны в списке.`;
...
Для КАЖДОГО дочернего продукта реши строго: есть ли в источниках КОНКРЕТНОЕ описание его СЛЕДУЮЩЕГО передела в указанном направлении? Описание текущего шага («${parentProduct}» → дети) или процесса родителя НЕ считается. Если такого описания нет — продукт в "insufficient". Сомнения — тоже в "insufficient".
